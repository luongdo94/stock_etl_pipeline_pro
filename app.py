    ...
    if p_tickers:
        ...
        if _pf_cohere_key:
            ...
            if _stored_review and _stored_review.get("report"):
                ...
                # ── Step 3: Simulate Rebalance Trigger ──
                st.markdown("---")
                _sim_col, _sim_info_col = st.columns([2, 5])
                with _sim_col:
                    _run_simulate = st.button(
                        "🧪 Simulate AI Proposed Portfolio",
                        type="primary",
                        use_container_width=True,
                        key="run_simulate_rebalance_btn",
                        help="Let AI propose an optimized share allocation and backtest it against your current portfolio."
                    )
                with _sim_info_col:
                    st.caption("AI will generate a JSON-structured rebalancing proposal based on Quality Scores and structural fundamentals. You can then backtest the proposed portfolio against your baseline.")

                if _run_simulate:
                    _saved_payload = _stored_review.get("payload", {})
                    _saved_macro_ctx = _stored_review.get("macro_ctx", "")
                    
                    from etl.llm_parser import get_rebalance_directives
                    # Pass current shares into positions so AI knows current holdings
                    _sim_positions = []
                    for _sp in _saved_payload.get("positions", []):
                        _t = _sp["ticker"]
                        _sp_enriched = dict(_sp)
                        _sp_enriched["shares"] = float(st.session_state.portfolio_shares.get(_t, 0))
                        _sim_positions.append(_sp_enriched)
                    _sim_payload = dict(_saved_payload)
                    _sim_payload["positions"] = _sim_positions

                    # known_tickers = all tickers in local DB (edge case filter)
                    _known_tickers = list(m_df["Ticker"].unique()) if not m_df.empty else []

                    with st.spinner("AI is computing optimal share allocation..."):
                        _sim_result = get_rebalance_directives(
                            _pf_cohere_key,
                            _sim_payload,
                            known_tickers=_known_tickers,
                            macro_context=_saved_macro_ctx,
                        )

                    if _sim_result.get("error"):
                        st.error(f"❌ Simulation failed: {_sim_result['error']}")
                    elif not _sim_result.get("proposed_portfolio"):
                        st.warning("⚠️ AI returned an empty portfolio proposal. Try running the CIO Review first.")
                    else:
                        st.session_state["pending_rebalance_portfolio"] = _sim_result
                        st.toast("✅ Simulation ready! Navigate to the Backtest tab to compare portfolios.", icon="🧪")
                        st.rerun()
...
if active_tab == "5. Backtest Lab":
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # STEP 4: Portfolio Simulation Override (A/B Comparison)
    # ═══════════════════════════════════════════════════════════════════════════
    _pending_sim = st.session_state.get("pending_rebalance_portfolio")
    if _pending_sim and _pending_sim.get("proposed_portfolio"):
        st.info(
            "🧪 **Simulation Mode Active** — You are previewing an AI-proposed portfolio rebalancing. "
            "The chart below compares your **Current Portfolio** (Baseline) vs the **AI Proposed Portfolio**. "
            "Review the metrics and click **Approve & Execute Rebalance** to apply the changes."
        )
        _sim_clear_col, _ = st.columns([1, 4])
        with _sim_clear_col:
            if st.button("✖ Clear Simulation", key="clear_sim_btn"):
                st.session_state["pending_rebalance_portfolio"] = None
                st.rerun()

        st.markdown(f"**AI Rationale:** {_pending_sim.get('rationale', 'N/A')}")
        st.markdown("---")

        # ── Build Proposed Portfolio returns ───────────────────────────────────
        _prop = _pending_sim["proposed_portfolio"]       # [{ticker, target_shares}, ...]
        _prop_tickers = [p["ticker"] for p in _prop if p["target_shares"] > 0]
        _prop_shares  = {p["ticker"]: p["target_shares"] for p in _prop if p["target_shares"] > 0}

        # Current (Baseline) portfolio
        _base_tickers = st.session_state.get("portfolio_tickers", [])
        _base_shares  = st.session_state.get("portfolio_shares", {})

        # Get latest prices for both portfolios
        _all_sim_tickers = list(set(_base_tickers) | set(_prop_tickers))
        _sim_prices_sub  = prices_full[prices_full["ticker"].isin(_all_sim_tickers)]
        _sim_ret_matrix  = (
            _sim_prices_sub
            .pivot(index="date", columns="ticker", values="daily_return_pct")
            .ffill().fillna(0) / 100
        )
        # Restrict to active date horizon
        _sim_ret_matrix = _sim_ret_matrix[_sim_ret_matrix.index.isin(prices["date"].unique())]

        def _compute_cum_returns(tickers, shares_dict, ret_matrix):
            """Return daily portfolio equity curve (normalized to 1.0 at start)."""
            if not tickers:
                return pd.Series(dtype=float)
            _tickers_avail = [t for t in tickers if t in ret_matrix.columns]
            if not _tickers_avail:
                return pd.Series(dtype=float)
            _sub = ret_matrix.reindex(columns=_tickers_avail, fill_value=0)
            _shares_arr = pd.Series({t: shares_dict.get(t, 0) for t in _tickers_avail})
            # Get latest prices to compute market values for weighting
            _lp = prices_full[prices_full["ticker"].isin(_tickers_avail)].groupby("ticker")["price_close"].last()
            _mv = _shares_arr * _lp.reindex(_tickers_avail).fillna(0)
            _total_mv = _mv.sum()
            if _total_mv <= 0:
                return pd.Series(dtype=float)
            _w = (_mv / _total_mv).values
            _port_daily = (_sub * _w).sum(axis=1)
            return (1 + _port_daily).cumprod()

        _base_cum  = _compute_cum_returns(_base_tickers, _base_shares, _sim_ret_matrix)
        _prop_cum  = _compute_cum_returns(_prop_tickers, _prop_shares, _sim_ret_matrix)

        # ── Step 5: Comparative Chart (A/B) ────────────────────────────────────
        if not _base_cum.empty or not _prop_cum.empty:
            fig_ab = go.Figure()
            if not _base_cum.empty:
                fig_ab.add_trace(go.Scatter(
                    x=_base_cum.index, y=_base_cum.values * 100 - 100,
                    name="📁 Current Portfolio (Baseline)",
                    line=dict(color="rgba(255,255,255,0.5)", width=2, dash="dot")
                ))
            if not _prop_cum.empty:
                fig_ab.add_trace(go.Scatter(
                    x=_prop_cum.index, y=_prop_cum.values * 100 - 100,
                    name="🤖 AI Proposed Portfolio",
            ...
            # ── Delta Metrics Table ─────────────────────────────────────────────
            def _calc_metrics(cum_s):
                if cum_s.empty:
                    return {"Return": 0, "Max DD": 0, "Volatility": 0, "Sharpe": 0}
                daily = cum_s.pct_change().fillna(0)
                ret   = float((cum_s.iloc[-1] - 1) * 100)
                dd    = float((cum_s / cum_s.cummax() - 1).min() * 100)
                vol   = float(daily.std() * (252 ** 0.5) * 100)
                excess = daily - 0.04 / 252
                sharpe = float(excess.mean() / excess.std() * (252 ** 0.5)) if excess.std() > 0 else 0
                return {"Return (%)": round(ret, 2), "Max DD (%)": round(dd, 2), "Volatility (%)": round(vol, 2), "Sharpe": round(sharpe, 2)}

            _base_m = _calc_metrics(_base_cum)
            _prop_m = _calc_metrics(_prop_cum)
            _delta  = {k: round(_prop_m[k] - _base_m[k], 2) for k in _base_m}

            _delta_df = pd.DataFrame({
                "Metric":    list(_base_m.keys()),
                "Baseline":  list(_base_m.values()),
                "AI Proposed": list(_prop_m.values()),
                "Δ (AI - Baseline)": list(_delta.values()),
            })
            st.dataframe(_delta_df, hide_index=True, use_container_width=True,
                         column_config={
                             "Δ (AI - Baseline)": st.column_config.NumberColumn("Δ (AI - Baseline)", format="%.2f"),
                             "Baseline": st.column_config.NumberColumn("Baseline", format="%.2f"),
                             "AI Proposed": st.column_config.NumberColumn("AI Proposed", format="%.2f"),
                         })

            # ── Step 6: Approve & Execute ───────────────────────────────────────
            st.markdown("---")
            _approve_col, _desc_col = st.columns([2, 5])
            with _approve_col:
                if st.button("✅ Approve & Execute Rebalance", type="primary", use_container_width=True, key="approve_rebalance_btn"):
                    # Override session state with proposed shares
                    _new_shares = {p["ticker"]: p["target_shares"] for p in _prop if p["target_shares"] > 0}
                    _new_tickers = list(_new_shares.keys())
                    # Retain cost basis where ticker already existed; default to 0 for new
                    _new_costs = {t: st.session_state.get("portfolio_cost", {}).get(t, 0.0) for t in _new_tickers}
                    st.session_state.portfolio_tickers = _new_tickers
                    st.session_state.portfolio_shares  = _new_shares
                    st.session_state.portfolio_cost    = _new_costs
                    st.session_state["_portfolio_version"] = st.session_state.get("_portfolio_version", 0) + 1
                    # Persist to Supabase
                    save_portfolio_to_db(_new_shares, _new_costs)
                    # Clear simulation state
                    st.session_state["pending_rebalance_portfolio"] = None
                    st.session_state["portfolio_cio_review"] = None  # Force fresh review
                    st.toast("🚀 Portfolio rebalanced and saved to database!", icon="✅")
                    st.rerun()
            with _desc_col:
                st.caption("Clicking **Approve** will overwrite your current portfolio with the AI-proposed allocations and sync to Supabase. This action can be undone by manually editing the Portfolio Builder tab.")

        st.markdown("---")
        render_header("activity", "Single-Asset Strategy Backtester")
    else:
        st.markdown("""
